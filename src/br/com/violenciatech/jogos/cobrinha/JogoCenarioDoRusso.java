...
import br.com.violenciatech.jogos.cobrinha.base.Elemento;
import br.com.violenciatech.jogos.cobrinha.base.Texto;
...
public class JogoCenarioDoRusso extends CenarioPadrao {
	...
	public void atualizar() {
		...
		if (!moveu) {
			if (dy!=0) {
				if (Jogo.controleTecla[Jogo.Tecla.ESQUERDA.ordinal()]) dx = -1;
				else if (Jogo.controleTecla[Jogo.Tecla.DIREITA.ordinal()]) dx = 1;
				...
				if (Jogo.controleTecla[Jogo.Tecla.CIMA.ordinal()]) dy = -1;
				else if (Jogo.controleTecla[Jogo.Tecla.BAIXO.ordinal()]) dy = 1;
		...
		} else temporizador += serpente.getVel();

		if (estado==Estado.JOGANDO && (blocoPorTela - contadorRastro)>0) while(!adicionaProximaFruta());
	}
	...
	public void desenhar(Graphics2D g) {
		if (fruta.isAtivo()) fruta.desenha(g);
		...
		if (estado!=Estado.JOGANDO) {
			...
			if (estado==Estado.GANHOU) texto.desenha(g, "Ganhou!", 180, 180);
			else texto.desenha(g, "Vixe!", 180, 180);
		} else if (Jogo.pausado) Jogo.textoPausa.desenha(g, "PAUSA", largura/2 - Jogo.textoPausa.getFonte().getSize(), altura/2);
	}
}