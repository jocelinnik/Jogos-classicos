...
import br.com.violenciatech.jogos.pacmansprites.Legume.Modo;
import br.com.violenciatech.jogos.pacmansprites.base.CenarioPadrao;
import br.com.violenciatech.jogos.pacmansprites.base.Elemento;
import br.com.violenciatech.jogos.pacmansprites.base.Texto;
import br.com.violenciatech.jogos.pacmansprites.base.Util;

public class JogoCenario extends CenarioPadrao {
	public enum Estado { JOGANDO, GANHOU, PERDEU }
	public enum Direcao { NORTE, SUL, OESTE, LESTE; }
	...
	private final ImageIcon azeitona = new ImageIcon("imagens/pacmansprites/azeitona.png");
	private final ImageIcon pepperoni = new ImageIcon("imagens/pacmansprites/pepperoni.png");
	private final ImageIcon cenario = new ImageIcon("imagens/pacmansprites/tiles_cenario.png");

	private int largEl;
	public static final int ESPACO_TOPO = 25;
	...
	public void atualizar() {
		if (estado == Estado.PERDEU) return;
		if (pausaColisao > 0) {
			pausaColisao--;
			return;
		}
		...
		if (Jogo.controleTecla[Jogo.Tecla.ESQUERDA.ordinal()]) prxDirecao = Direcao.OESTE;
		else if (Jogo.controleTecla[Jogo.Tecla.DIREITA.ordinal()]) prxDirecao = Direcao.LESTE;
		else if (Jogo.controleTecla[Jogo.Tecla.CIMA.ordinal()]) prxDirecao = Direcao.NORTE;
		else if (Jogo.controleTecla[Jogo.Tecla.BAIXO.ordinal()]) prxDirecao = Direcao.SUL;
	...
	private boolean validaDirecao(Direcao dir, Elemento el) {
		if (dir == Direcao.OESTE && validaMovimento(el, -1, 0)) return true;
		else if (dir == Direcao.LESTE && validaMovimento(el, 1, 0)) return true;
		else if (dir == Direcao.NORTE && validaMovimento(el, 0, -1)) return true;
		else if (dir == Direcao.SUL && validaMovimento(el, 0, 1)) return true;
	...
	private boolean validaMovimento(Elemento el, int dx, int dy) {
		...
		if (foraDaGrade(col, lin) || foraDaGrade(colLarg, linAlt)) return true;
		...
		if (el instanceof Legume) {
			if (grade[lin][col] == Nivel.LN || grade[lin][colLarg] == Nivel.LN || grade[linAlt][col] == Nivel.LN || grade[linAlt][colLarg] == Nivel.LN) {
				if (el.isAtivo() || ((Legume) el).getModo() == Modo.PRESO) return false;
	...
	private void atualizaDirecao(Pizza el) {
		...
		if (validaDirecao(direcao, el)) {
			if (direcao == Direcao.OESTE) tempDx = -1;
			else if (direcao == Direcao.LESTE) tempDx = 1;

			if (direcao == Direcao.NORTE) tempDy = -1;
			else if (direcao == Direcao.SUL) tempDy = 1;
		}

		if (!validaMovimento(el, tempDx, tempDy)) tempDx = tempDy = 0;
	...
	private void atualizaDirecaoInimigos(Legume el) {
		...
		if (el.getModo() == Legume.Modo.PRESO) {
			if (el.getDirecao() == Direcao.SUL && !validaDirecao(Direcao.SUL, el)) el.setDirecao(Direcao.NORTE);
			else if (el.getDirecao() == Direcao.NORTE && !validaDirecao(Direcao.NORTE, el)) el.setDirecao(Direcao.SUL);
			else if (el.getDirecao() != Direcao.NORTE && el.getDirecao() != Direcao.SUL) el.setDirecao(Direcao.NORTE);
			...
			if (lin > yLin && validaDirecao(Direcao.NORTE, el)) el.setDirecao(Direcao.NORTE);
			else if (lin < yLin && validaDirecao(Direcao.SUL, el)) el.setDirecao(Direcao.SUL);
			else if (col < xCol && validaDirecao(Direcao.LESTE, el)) el.setDirecao(Direcao.LESTE);
			else if (col > xCol && validaDirecao(Direcao.OESTE, el)) el.setDirecao(Direcao.OESTE);
			...
			if (el.isAtivo() && perdido) {
				...
				if (xCol < col && validaDirecao(Direcao.OESTE, el)) tempDir = Direcao.OESTE;
				else if (xCol > col && validaDirecao(Direcao.LESTE, el)) tempDir = Direcao.LESTE;
			} else {
				/* direcao = OESTE ou LESTE */
				if (yLin < lin && validaDirecao(Direcao.NORTE, el)) tempDir = Direcao.NORTE;
				else if (yLin > lin && validaDirecao(Direcao.SUL, el)) tempDir = Direcao.SUL;
			}

			if (tempDir != null && validaDirecao(tempDir, el)) el.setDirecao(tempDir);
			else if (!validaDirecao(el.getDirecao(), el)) el.setDirecao(sorteiaDirecao());
			...
			if (tempDir != null && validaDirecao(tempDir, el)) el.setDirecao(tempDir);
			else if (!validaDirecao(el.getDirecao(), el)) el.setDirecao(trocaDirecao(el.getDirecao()));

			if (col == xCol && lin == yLin) el.setModo(Legume.Modo.INATIVO);
			...
			else if (col == xCol && lin == yLin) el.setModo(Legume.Modo.PRESO);
		}

		if (validaDirecao(el.getDirecao(), el)) {
			if (el.getDirecao() == Direcao.NORTE) tempDy = -1;
			else if (el.getDirecao() == Direcao.SUL) tempDy = 1;
			else if (el.getDirecao() == Direcao.OESTE) tempDx = -1;
			else if (el.getDirecao() == Direcao.LESTE) tempDx = 1;
		}
	...
	private Direcao trocaDirecao(Direcao direcao) {
		if (direcao == Direcao.NORTE) return Direcao.OESTE;
		else if (direcao == Direcao.OESTE) return Direcao.SUL;
		else if (direcao == Direcao.SUL) return Direcao.LESTE;
	...
	private boolean foraDaGrade(int coluna, int linha) {
		if (linha < 0 || linha >= grade.length) return true;
		if (coluna < 0 || coluna >= grade[0].length) return true;

		return false;
	}

	private boolean foraDaTela(Elemento el) {
		if (el.getPx() < 0 || el.getPx() + el.getLargura() > largura) return true;
		if (el.getPy() < 0 || el.getPy() + el.getAltura() > altura) return true;
	...
	private void corrigePosicao(Elemento el) {
		...
		if (el.getDx() == 0 && novaPx != col) novaPx = col;
		else if (el.getPx() + largEl < 0) novaPx = largura;
		else if (el.getPx() > largura) novaPx = -largEl;

		if (el.getDy() == 0 && novaPy != lin) novaPy = lin;
		else if (el.getPy() + largEl < 0) novaPy = altura;
		else if (el.getPy() > altura) novaPy = -largEl;
	...
	private void superPizza(boolean modoSuper) {
		...
		for (Legume el : inimigos) {
			...
			if (modoSuper && el.getModo() == Legume.Modo.CACANDO) el.setModo(Legume.Modo.FUGINDO);
			else if (!modoSuper && el.getModo() == Legume.Modo.FUGINDO) el.setModo(Legume.Modo.CACANDO);
		}
	}
		...
		for (int lin=0;lin<grade.length;lin++) {
			for (int col=0;col<grade[0].length;col++) {
				...
				if (valor >= 0) {
					...
					if (valor == Nivel.CN) g.drawImage(azeitona.getImage(), col * largEl, lin * largEl + ESPACO_TOPO, null);
					else if (valor == Nivel.SC) g.drawImage(pepperoni.getImage(), col * largEl, lin * largEl + ESPACO_TOPO, null);
				}
			}
		}
