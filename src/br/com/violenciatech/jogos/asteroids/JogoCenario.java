...
import br.com.violenciatech.jogos.asteroids.base.Texto;
...
public class JogoCenario extends CenarioPadrao{
	...
	public void carregar() {
		...
		for (int i=0;i<aerolitos.length;i++) aerolitos[i] = new Asteroide();
	...
	public void atualizar() {
		...
		if (Jogo.controleTecla[Jogo.Tecla.ESQUERDA.ordinal()]) graus -= 10;
		else if (Jogo.controleTecla[Jogo.Tecla.DIREITA.ordinal()]) graus += 10;

		if (graus < 0) graus += 360;
		else if (graus > 360) graus -= 360;
		...
		for (int i=0;i<aerolitos.length;i++) {
			...
			switch (Jogo.nivel) {
				case 0:
					...
					break;
				case 1:
					if (ast.getPy() <= 0) ast.setAngulo(90);
					else ast.setAngulo(270);
					break;
				default:
