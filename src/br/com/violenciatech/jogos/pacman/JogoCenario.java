...
import br.com.violenciatech.jogos.pacman.Pizza.Modo;
import br.com.violenciatech.jogos.pacman.base.CenarioPadrao;
import br.com.violenciatech.jogos.pacman.base.Elemento;
import br.com.violenciatech.jogos.pacman.base.Texto;
...
public class JogoCenario extends CenarioPadrao {
	...
	private void atualizaDirecaoInimigos(Pizza el) {
		...
		if (el.getModo() == Pizza.Modo.PRESO) {
			...
			if (temporizadorFantasma > 50) el.setModo(Pizza.Modo.ATIVO);
			else temporizadorFantasma++;
			...
			if (col == xCol && lin == yLin) el.setModo(Pizza.Modo.INATIVO);
			...
			else if (col == xCol && lin == yLin) el.setModo(Pizza.Modo.PRESO);
		}
	...
	private void comePastilha(Elemento el) {
		...
		if (foraDaGrade(col, lin)) return;
		if (grade[lin][col] == Nivel.CN || grade[lin][col] == Nivel.SC) {
			pontos += grade[lin][col] == Nivel.CN ? 5 : 25;
			totalPastilha--;

			if (totalPastilha == 0) estado = JogoCenario.Estado.GANHOU;
			else if (grade[lin][col] == Nivel.SC) superPizza(true);
	...
	private void superPizza(boolean modoSuper) {
		...
		for (Pizza el : inimigos) {
			if (el == null) continue;
			if (modoSuper && el.getModo() == Pizza.Modo.CACANDO) el.setModo(Pizza.Modo.FUGINDO);
			else if (!modoSuper && el.getModo() == Pizza.Modo.FUGINDO) el.setModo(Pizza.Modo.CACANDO);
		}
	}
