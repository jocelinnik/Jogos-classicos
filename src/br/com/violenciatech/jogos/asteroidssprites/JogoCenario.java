...
import br.com.violenciatech.jogos.asteroidssprites.base.CenarioPadrao;
...
import br.com.violenciatech.jogos.asteroidssprites.base.Texto;
import br.com.violenciatech.jogos.asteroidssprites.base.Util;
...
public class JogoCenario extends CenarioPadrao{
		...
		for (int i=0;i<contadorAsteroides;i++) {
			...
			for (Tiro tiro : tiros) {
				if (Util.colide(ast, tiro)) {
					...
					if (ast.divide()) novaParteAsteroide(ast);
					else ast.setAtivo(false);
	...
	public void controlaJogadorUm() {
		...
		if (Jogo.controleTecla[Jogo.Tecla.ESQUERDA.ordinal()]) graus -= Nave.ROTACAO_VEL;
		else if (Jogo.controleTecla[Jogo.Tecla.DIREITA.ordinal()]) graus += Nave.ROTACAO_VEL;
	...
	private void adicionarTiro(Nave jogador) {
		if (contadorTiro > 1) contadorTiro--;
		else contadorTiro = tiros.length - 1;
	...
	public void desenhar(Graphics2D g) {
		g.setTransform(AF_VAZIO);

		if (ampliar) {
			g.scale(1 + escala, 1 + escala);
			g.translate(-largura * escala/2f + 1, -altura * escala/2f + 1);
		} else if (reduzir) {
		...
		texto.desenha(g, "Tup | " + navJogUm.getPontos(), 10, 20);
		texto.desenha(g, "Easy | " + navJogDois.getPontos(), largura - 120, 20);

		if (navJogUm.getSeguidos()>2) texto.desenha(g, "x" + navJogUm.getSeguidos(), 10, 40);
		if (navJogDois.getSeguidos() > 2) texto.desenha(g, "x" + navJogDois.getSeguidos(), largura - 120, 40);

		for (int i = 0; i < tiros.length; i++) {
			if (tiros[i].isAtivo()) tiros[i].desenha(g);
		}

		for (int i = 0; i < contadorAsteroides; i++) {
			if (aerolitos[i].isAtivo()) aerolitos[i].desenha(g);
		}
