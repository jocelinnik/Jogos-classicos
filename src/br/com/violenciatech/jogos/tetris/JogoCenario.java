...
import br.com.violenciatech.jogos.tetris.base.Texto;

public class JogoCenario extends CenarioPadrao {
	enum Estado { JOGANDO, GANHOU, PERDEU }
	...
	public void carregar() {
		...
		for(Type t : audioFileTypes) System.out.println(t.getExtension());

		try {
			as = AudioSystem.getAudioInputStream(new File("som/tetris/adiciona_peca.wav"));
			clipAdicionarPeca = AudioSystem.getClip();
			clipAdicionarPeca.open(as);

			as = AudioSystem.getAudioInputStream(new File("som/tetris/109662_grunz_success.wav"));
			clipMarcarLinha = AudioSystem.getClip();
			clipMarcarLinha.open(as);

			seqSomDeFundo = MidiSystem.getSequencer();
			seqSomDeFundo.setSequence(MidiSystem.getSequence(new File("som/tetris/piano_quebrado.mid")));
			seqSomDeFundo.open();
	...
	public void atualizar() {
		...
		if (Jogo.controleTecla[Jogo.Tecla.ESQUERDA.ordinal()]) {
			if (validaMovimento(peca, ppx - 1, ppy)) ppx--;
		...
		if (Jogo.controleTecla[Jogo.Tecla.CIMA.ordinal()]) {
			...
			if (validaMovimento(peca, ppx, ppy + 1)) ppy++;
		}

		if (depurar && Jogo.controleTecla[Jogo.Tecla.BC.ordinal()]) {
			if (++idPeca == Peca.PECAS.length) idPeca = 0;
		...
		if (animar && temporizador >= 5) {
			...
			if (colidiu(ppx, ppy + 1)) {
				...
				if (!parouForaDaGrade()) {
					...
					peca = null;
					if (!animar) adicionaPeca();
				} else {
					estado = Estado.PERDEU;
				}
			}else ppy++;
		} else temporizador += nivel;
	}
	...
	public void adicionaPeca() {
		...
		// Primeira chamada
		if (idPeca == -1) idPeca = rand.nextInt(Peca.PECAS.length);
		else idPeca = idPrxPeca;
		
		idPrxPeca = rand.nextInt(Peca.PECAS.length);

		// Isso acontece muito
		if (idPeca == idPrxPeca) idPrxPeca = rand.nextInt(Peca.PECAS.length);
	...
	private void adicionarPecaNaGrade() {
		for (int col = 0; col < peca.length; col++) {
			for (int lin = 0; lin < peca[col].length; lin++) {
				if (peca[lin][col] != 0) grade[col + ppx][lin + ppy] = idPeca;
			}
		}
	}
	...
	private boolean validaMovimento(int[][] peca, int px, int py) {
		...
		for (int col = 0; col < peca.length; col++) {
			for (int lin = 0; lin < peca[col].length; lin++) {
				if (peca[lin][col] == 0) continue;

				int prxPx = col + px; // Proxima posicao peca x
				int prxPy = lin + py; // Proxima posicao peca y

				if (prxPx < 0 || prxPx >= grade.length) return false;
				if (prxPy >= grade[0].length) return false;
				if (prxPy < 0) continue;

				// Colidiu com uma peca na grade
				if (grade[prxPx][prxPy] > ESPACO_VAZIO) return false;
			}
		}
	...
	private boolean colidiu(int px, int py) {
		...
		for (int col = 0; col < peca.length; col++) {
			for (int lin = 0; lin < peca[col].length; lin++) {
				...
				if (prxPy == grade[0].length) return true;

				// Fora da grade
				if (prxPy < 0) continue;

				// Colidiu com uma peca na grade
				if (grade[prxPx][prxPy] > ESPACO_VAZIO) return true;
			}
		}
	...
	private boolean marcarLinha() {
		...
		for (int lin = grade[0].length - 1; lin >= 0; lin--) {
			...
			if (linhaCompleta) {
				multPontos++;
				for (int col = grade.length - 1; col >= 0; col--) grade[col][lin] = LINHA_COMPLETA;
			}
		}
			...
			for (int lin = grade[0].length - 1; lin >= 0; lin--) {
				if (grade[col][lin] == LINHA_COMPLETA) {
					...
					for (; prxLinha > -1; prxLinha--) {
						if (grade[col][prxLinha] == LINHA_COMPLETA) continue;
						else break;
					}

					for (; moverPara > -1; moverPara--, prxLinha--) {
						if (prxLinha > -1) grade[col][moverPara] = grade[col][prxLinha];
						else grade[col][moverPara] = ESPACO_VAZIO;
					}
				}
			}
		}
	...
	protected void girarPeca(boolean sentidoHorario) {
		...
		for (int i = 0; i < peca.length; i++) {
			for (int j = 0; j < peca.length; j++) {
				if (sentidoHorario) temp[j][peca.length - i - 1] = peca[i][j];
				else temp[peca.length - j - 1][i] = peca[i][j];
			}
		}
		...
		if (validaMovimento(temp, ppx, ppy)) peca = temp;
	}
	...
	private void girarReposicionarPeca(boolean sentidoHorario) {
		...
		for (int i = 0; i < peca.length; i++) {
			for (int j = 0; j < peca.length; j++) {
				if (sentidoHorario) tempPeca[j][peca.length - i - 1] = peca[i][j];
				else tempPeca[peca.length - j - 1][i] = peca[i][j];
			}
		}
		...
		for (int i = 0; i < tempPeca.length; i++) {
			for (int j = 0; j < tempPeca.length; j++) {
				if (tempPeca[j][i] == 0) continue;

				int prxPx = i + tempPx;

				if (prxPx < 0)tempPx = tempPx - prxPx;
				else if (prxPx == grade.length) tempPx = tempPx - 1;
			}
		}
	...
	public void desenhar(Graphics2D g) {
		for (int col = 0; col < grade.length; col++) {
			for (int lin = 0; lin < grade[0].length; lin++) {
				...
				if (valor == ESPACO_VAZIO) continue;
				if (valor == LINHA_COMPLETA) g.setColor(Color.RED);
				else g.setColor(Peca.Cores[valor]);
		...
		if (estado != Estado.JOGANDO) {
			texto.setCor(Color.WHITE);
			if (estado == Estado.GANHOU) texto.desenha(g, "Finalmente!", 180, 180);
			else texto.desenha(g, "Deu ruim!", 180, 180);
		}
	}
}