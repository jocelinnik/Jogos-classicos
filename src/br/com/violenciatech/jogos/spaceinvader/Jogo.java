...
import br.com.violenciatech.jogos.spaceinvader.Invader.Tipos;
import br.com.violenciatech.jogos.spaceinvader.base.Elemento;
import br.com.violenciatech.jogos.spaceinvader.base.Texto;
import br.com.violenciatech.jogos.spaceinvader.base.Util;
...
public class Jogo extends JFrame {
	...
	private void setaTecla(int tecla, boolean pressionada) {
		switch (tecla) {
			case KeyEvent.VK_UP: controleTecla[0] = pressionada; break;
			case KeyEvent.VK_DOWN: controleTecla[1] = pressionada; break;
			case KeyEvent.VK_LEFT: controleTecla[2] = pressionada; break;
			case KeyEvent.VK_RIGHT: controleTecla[3] = pressionada; break;
			case KeyEvent.VK_SPACE: controleTecla[4] = pressionada; break;
		}
	}
	...
	public void carregarJogo() {
		...
		for (int i = 0; i < tiros.length; i++) tiros[i] = new Tiro(true);
	...
	public void iniciarJogo() {
		...
		while (true) {
			if (System.currentTimeMillis() >= prxAtualizacao) {
				...
				if (tanque.isAtivo()) {
					if (controleTecla[2]) tanque.setPx(tanque.getPx() - tanque.getVel());
					else if (controleTecla[3]) tanque.setPx(tanque.getPx() + tanque.getVel());
				}
				...
				for (int j = invasores[0].length - 1; j >= 0; j--) {
					for (int i = 0; i < invasores.length; i++) {
						...
						if (moverInimigos) {
							inv.atualiza();
							if (novaLinha) inv.setPy(inv.getPy() + inv.getAltura() + espacamento);
							else inv.incPx(espacamento * dir);

							if (!novaLinha && !colideBordas) {
								int pxEsq = inv.getPx() - espacamento;
								int pxDir = inv.getPx() + inv.getLargura() + espacamento;

								if (pxEsq <= 0 || pxDir >= tela.getWidth()) colideBordas = true;
							}

							if (!tiros[0].isAtivo() && inv.getPx() < tanque.getPx()) addTiroInimigo(inv, tiros[0]);
							else if (!tiros[1].isAtivo() && inv.getPx() > tanque.getPx() && inv.getPx() < tanque.getPx() + tanque.getLargura()) addTiroInimigo(inv, tiros[1]);
							else if (!tiros[2].isAtivo() && inv.getPx() > tanque.getPx()) addTiroInimigo(inv, tiros[2]);
